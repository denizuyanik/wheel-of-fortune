...
// 1. Add windParticles to constructor
code = code.replace(
...
// Remove duplicate old center-hub-btn CSS
code = code.replace(/\.center-hub-btn\s*\{[\s\S]*?\.center-hub-btn:disabled\s*\{[\s\S]*?\}/, "");

// 3. Update drawWheel to draw permanent 3D Metallic Center Hub directly at (center, center) on Canvas
...
// 4. Add spawnWindGust() and drawWind() methods
const windMethods = `  spawnWindGust() {
...
code = code.replace("  applyBoost() {", windMethods + "\n  applyBoost() {");
...
console.log("SUCCESS: Added Wind Gust Turbo Effect & Locked Canvas Center Hub in wheel-widget.js!");
