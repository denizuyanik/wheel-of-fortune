...
<body>
  ...
  <main class="sandbox-main">
    ...
    <div class="dashboard-view" id="view-dashboard">
      ...
      <div class="dash-header">
        <div style="display:flex; align-items:center; gap:20px; flex-wrap:wrap;">
          <div>
            ...
            <p id="dash-sub-txt" style="margin:4px 0 0 0; font-size:13px; color:#94a3b8;">Manage wheel prizes, win odds, CRM leads, theme, typography, and 16-language texts</p>
          </div>
      ...
      <div class="dash-nav" id="dash-nav-tabs">
        ...
        <button type="button" class="dash-tab-btn" data-pane="dash-theme" id="tab-nav-theme">🎨 Theme & Typography</button>
      ...
      <!-- Tab 2: UNLIMITED Prize Slices -->
      <div class="dash-content-pane" id="dash-prizes">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px; flex-wrap:wrap; gap:10px;">
          <div>
            <h3 id="lbl-prizes-title" style="margin:0 0 4px 0; color:#fbbf24;">🎡 Wheel Segments & Probability Settings</h3>
            <p id="lbl-prizes-sub" style="font-size:13px; color:#94a3b8; margin:0;">Add unlimited prize slices, pick custom background colors, text colors, codes and win odds:</p>
          </div>
          <div style="display:flex; gap:10px; align-items:center;">
            <span id="prizes-total-odds-badge" style="background:#10b981; color:#fff; font-weight:700; padding:6px 12px; border-radius:8px; font-size:13px;">Total: 100%</span>
            <button type="button" style="background:#f59e0b; border:none; color:#0f172a; font-weight:800; padding:8px 16px; border-radius:8px; cursor:pointer;" id="btn-add-slice">+ Add New Slice</button>
          </div>
        </div>
        <table class="dash-table">
          <thead>
            <tr>
              <th id="th-bg-color">BG Color</th>
              <th id="th-txt-color">Text Color</th>
      ...
      <div class="dash-content-pane" id="dash-theme">
        <div style="margin-bottom:12px;">
          <h3 id="lbl-theme-title" style="margin:0 0 4px 0; color:#fbbf24;">🎨 Visual Theme, Typography & Anti-Cheat Rules</h3>
          <p id="lbl-theme-sub" style="font-size:13px; color:#94a3b8; margin:0;">Select visual color palette, custom Google font typography, and daily spin limits:</p>
        </div>
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(240px, 1fr)); gap:16px;">
          ...
          <div style="background:rgba(0,0,0,0.3); padding:16px; border-radius:12px;">
            <label style="font-size:13px; font-weight:700; color:#fbbf24; display:block; margin-bottom:8px;">🔤 Typography & Font Family</label>
            <select style="width:100%; padding:8px;" id="dash-font-sel" class="ctrl-select">
              <option value="Poppins" selected>Poppins (Modern Rounded - Default)</option>
              <option value="Inter">Inter (Clean & Professional)</option>
              <option value="Outfit">Outfit (Bold & Elegant)</option>
              <option value="Montserrat">Montserrat (Geometric Luxury)</option>
              <option value="Roboto">Roboto (Universal Standard)</option>
              ...
              <option value="Orbitron">Orbitron (Futuristic Cyberpunk)</option>
              <option value="Comic Neue">Comic Neue (Playful & Casual)</option>
            </select>
          </div>
  ...
  <script type="module">
    ...
    // Header Persistent Language Selector & Font Selector
    const dashHeaderLangSel = document.getElementById("dash-header-lang-sel");
    const dashFontSel = document.getElementById("dash-font-sel");
    ...
    const totalOddsBadge = document.getElementById("prizes-total-odds-badge");
    const btnAddSlice = document.getElementById("btn-add-slice");

    function renderPrizesTable() {
      const sumOdds = currentPrizes.reduce((s, p) => s + (Number(p.probability) || 0), 0);
      totalOddsBadge.textContent = "Total: " + sumOdds + "%";
      totalOddsBadge.style.background = sumOdds === 100 ? "#10b981" : "#ef4444";

      prizesTbody.innerHTML = currentPrizes.map((p, idx) => \`
        <tr>
          <td><input type="color" value="\${p.color || '#6366f1'}" data-idx="\${idx}" class="slice-bg-color" style="width:34px; height:32px; padding:0; border:none; border-radius:4px; cursor:pointer;" title="Slice Color" /></td>
          <td><input type="color" value="\${p.textColor || '#ffffff'}" data-idx="\${idx}" class="slice-txt-color" style="width:34px; height:32px; padding:0; border:none; border-radius:4px; cursor:pointer;" title="Text Color" /></td>
          <td><input type="text" value="\${p.label.replace(/"/g, '&quot;')}" data-idx="\${idx}" class="slice-label ctrl-input" style="width:140px; padding:4px 8px;" /></td>
          <td><input type="text" value="\${(p.code || '').replace(/"/g, '&quot;')}" data-idx="\${idx}" class="slice-code ctrl-input" style="width:110px; padding:4px 8px;" placeholder="Optional code" /></td>
          <td><input type="number" value="\${p.probability}" data-idx="\${idx}" class="slice-prob ctrl-input" style="width:60px; padding:4px;" /> %</td>
          <td>
            <label style="display:flex; align-items:center; gap:4px; font-size:12px; cursor:pointer;">
              <input type="checkbox" data-idx="\${idx}" class="slice-win" \${p.isWinner !== false ? 'checked' : ''} />
              \${p.isWinner !== false ? '✅ Win' : '❌ No'}
            </label>
          </td>
          <td>
            <button type="button" data-idx="\${idx}" class="slice-del" style="background:\${currentPrizes.length <= 2 ? 'transparent' : '#fee2e2'}; color:\${currentPrizes.length <= 2 ? '#64748b' : '#ef4444'}; border:none; padding:4px 8px; border-radius:6px; font-weight:700; cursor:\${currentPrizes.length <= 2 ? 'not-allowed' : 'pointer'};" \${currentPrizes.length <= 2 ? 'disabled' : ''}>✕</button>
          </td>
        </tr>
      \`).join("");

      // Bind input events
      prizesTbody.querySelectorAll(".slice-bg-color").forEach((inp) => {
        inp.onchange = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].color = e.target.value;
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-txt-color").forEach((inp) => {
        inp.onchange = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].textColor = e.target.value;
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-label").forEach((inp) => {
        inp.oninput = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].label = e.target.value;
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-code").forEach((inp) => {
        inp.oninput = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].code = e.target.value;
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-prob").forEach((inp) => {
        inp.oninput = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].probability = Number(e.target.value) || 0;
          const sum = currentPrizes.reduce((s, p) => s + (Number(p.probability) || 0), 0);
          totalOddsBadge.textContent = "Total: " + sum + "%";
          totalOddsBadge.style.background = sum === 100 ? "#10b981" : "#ef4444";
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-win").forEach((inp) => {
        inp.onchange = (e) => {
          const idx = Number(e.target.getAttribute("data-idx"));
          currentPrizes[idx].isWinner = e.target.checked;
          renderPrizesTable();
          syncPrizesToWidget();
        };
      });

      prizesTbody.querySelectorAll(".slice-del").forEach((btn) => {
        btn.onclick = (e) => {
    ...
    btnAddSlice.onclick = () => {
      const palette = ["#6366f1", "#ec4899", "#f59e0b", "#10b981", "#8b5cf6", "#3b82f6", "#ef4444", "#14b8a6", "#e11d48", "#059669", "#7c3aed", "#d97706", "#2563eb", "#db2777"];
      const nextColor = palette[currentPrizes.length % palette.length];
      const newPrize = {
        id: "p_" + Date.now(),
        label: "Reward " + (currentPrizes.length + 1),
        code: "WIN" + (currentPrizes.length + 1),
        color: nextColor,
        textColor: "#ffffff",
        probability: 10,
        isWinner: true,
      ...
      renderPrizesTable();
      syncPrizesToWidget();
      log("➕ Added new prize segment: " + newPrize.label);
    };

    function syncPrizesToWidget() {
      widget.setAttribute("reward-pool", JSON.stringify(currentPrizes));
    }
    ...
    function updateWidget() {
      ...
      widget.setAttribute("font-family", selFont.value);
      widget.setAttribute("color-theme", selTheme.value);
      widget.setAttribute("daily-limit", selLimit.value);
      syncPrizesToWidget();
      log(\`Updated settings: Lang=\${selLang.value.toUpperCase()}, Font=\${selFont.value}, Theme=\${selTheme.value}, Limit=\${selLimit.value}\`);
    }
    ...
    selFont.addEventListener("change", (e) => {
      dashFontSel.value = e.target.value;
      updateWidget();
    });
    dashFontSel.addEventListener("change", (e) => {
      selFont.value = e.target.value;
      updateWidget();
    });
    ...
    function updateDashboardLanguage(lang) {
      ...
      document.getElementById("th-bg-color").textContent = "BG " + t.colColor;
      document.getElementById("th-txt-color").textContent = "Text Color";
    ...
    if (btnSaveDash) {
      btnSaveDash.addEventListener("click", () => {
        const lang = dashHeaderLangSel.value;
        const font = dashFontSel.value;
        const theme = document.getElementById("dash-theme-sel").value;
        const limit = document.getElementById("dash-limit-inp").value;
        const t = DASHBOARD_I18N[lang] || DASHBOARD_I18N.en;

        localStorage.setItem("wheel_of_fortune_custom_texts", JSON.stringify(customTextsState));
        widget.setAttribute("custom-texts", JSON.stringify(customTextsState));
        widget.setAttribute("lang", lang);
        widget.setAttribute("font-family", font);
        widget.setAttribute("color-theme", theme);
        widget.setAttribute("daily-limit", limit);
        syncPrizesToWidget();
...
console.log("Successfully wrote enhanced multilingual public/test-sandbox.html with unlimited slices and typography!");
