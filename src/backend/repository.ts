...
export async function findSpinByIdempotencyKey(idempotencyKey: string): Promise<SpinRecord | null> {
  const spins = await queryCollection<SpinRecord>(SPINS_COLLECTION, { idempotencyKey }, [], 1, { elevated: true, consistentRead: true });
  return spins[0] ?? null;
}
...
export async function recordSpin(spin: Omit<SpinRecord, '_id'>): Promise<SpinRecord> {
  return elevatedInsert(SPINS_COLLECTION, spin) as Promise<SpinRecord>;
}
